...
export const getServerSideProps: GetStaticProps = async ({ locale, params }) => {
    const assets = await fetchAssets({ _start: 0, _limit: 20 })
    const collections = await fetchCollections({ _start: 0, _limit: 20 });
    const users = await fetchUsers({ _start: 0, _limit: 20 })
